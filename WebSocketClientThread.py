...
class MyThread8_1(QThread):
    ...
    def run(self):
        try:
            ...
            self.mylogger8_1.error('---------------------Error happens in the run funciton of WebSocket Thread-------------------------', exc_info=True)
...
class MyThread8_1_klas(QThread):
    ...
    Compared with the MyThread8_1 class, the error singal is added;
    """
...
class MyThread8_3(QThread):
    ...
    Compared with the MyThread8_1 class, this class will emit payclear_success_code signal if the user pays by QR code;
    Compared with the MyThread8_1 class, the payclear_success and payclear_success_code signals will carry the order number;
    Compared with the MyThread8_1_klas class, this class adds the signal for zeroing weight;
    """
    payclear_success = pyqtSignal(object)  # order number,
    payclear_success_code = pyqtSignal(object)  # order number
    payclear_failure = pyqtSignal()
    opendoor = pyqtSignal(object)
    weight_clear = pyqtSignal()

    def __init__(self, logger_name='hobin', *, socketurl='http://sys.commaai.cn', socketport=60000, storename='development center for testing',
                 storeid='2', screenid='1', doorid='1', **kwargs):
        super(MyThread8_3, self).__init__()
        # the first way to configuring the parameters, almost all parameters get their value from those named keyword arguments.
        self.socketUrl = socketurl
        self.storeName = storename
        self.storeId = storeid
        ...
        # some variables
        self.mylogger8_3 = logging.getLogger(logger_name)
        self.msg = '{"store_id":"%s", "remark":"%s", "screen_id": "%s"}' % (self.storeId, self.storeName, self.screenId)
        self.mylogger8_3.info('Initialization of WebSocket thread is successful.')

    def on_connect(self):
        self.mylogger8_3.info('WebSocket-Client thread connects')

    def on_disconnect(self):
        """I guess the error information comes from the LoggingNamespace."""
        self.mylogger8_3.info('WebSocket-Client thread disconnects')

    def on_reconnect(self):
        self.socket.emit('join', self.storeId)
        self.mylogger8_3.info('WebSocket-Client thread reconnects')
    ...
    def on_msg_response(self, *args):
        ...
        """
        self.mylogger8_3.info(u'WebSocket-Client thread gets: %s' % args)
        try:
            if args[0].get('action') == 'opendoor' and self.doorId == args[0].get('door_id'):
                self.opendoor.emit(args[0]['door_id'])
            elif args[0].get('action') == 'payclear' and self.storeId == args[0].get('store_id') and self.screenId == args[0].get('screen_id'):
                if args[0].get('code') == '1':
                    if args[0].get('pay_model') == '1':
                        self.payclear_success_code.emit(args[0]['order_no'])
                    else:
                        # if pay_model is 2, the user finish the payment by gesture;
                        self.payclear_success.emit(args[0]['order_no'])
                elif args[0].get('code') == '0':
                    self.payclear_failure.emit()
            elif args[0].get('action') == 'clear_weight' and self.storeId == args[0].get('store_id') and self.screenId == args[0].get('screen_id'):
                self.weight_clear.emit()
        except BaseException:
            self.mylogger8_3.error('---------Error happens in the Websocket-------------', exc_info=True)
    ...
    def run(self):
        try:
            ...
            self.mylogger8_3.error('---------------------Error happens in the run funciton of WebSocket Thread-------------------------', exc_info=True)

    def ping_timer(self):
        """It is like heart-beating."""
        try:
            self.mylogger8_3.info('WebSocket-Client thread sends: %s' % self.msg)
            self.socket.emit('ping server', self.msg)
        except Exception as e:
            self.mylogger8_3.error(e)
            self.mylogger8_3.error('--------------Error happens in the ping_timer function of WebSocket Thread-------------------------', exc_info=True)
