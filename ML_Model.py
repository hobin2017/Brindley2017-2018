...
class Detection1_2_1(QThread):
    ...
    def run(self):
        ...
        image2 = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        image_np_expanded = np.expand_dims(image2, axis=0)  # inserting a new axis at the position that you provide
...
class Detection1_3(QObject):
    """
    It is used to detect products.
    Compared with the Detection1_2_1 class, the super class becomes QObject instead of QThread.
    Compared with the Detection1_2_1 class, it adds support to reopen the camera. And self.recent_error_timestamp is
    used to avoid the execution of 'self.recent_error_timestamp=0' when reading camera successfully.
    """
    detected = pyqtSignal(object)
    upload_img = pyqtSignal(object, object)
    start_work = pyqtSignal()  # Its slot is self.detecting_items()
    camera_error = pyqtSignal()  # it is emitted when camera error occurs
    camera_error_info_update = pyqtSignal(object)  # bool; designed to update the camera error for standby layout.

    def __init__(self, camera_object, logger_name='hobin', parent=None, *,
                 path_to_ckpt = 'D:\\01PythonFile\\Brindley\\object_detection\\output_inference_graph.pb\\frozen_inference_graph.pb',
                 path_to_labels = 'D:\\01PythonFile\\Brindley\\object_detection\\output_inference_graph.pb\\pascal_label_map.pbtxt',
                 num_classes=25, cam_path='0',  **kwargs):
        super(Detection1_3, self).__init__()
        # timer for reopen of camera
        self.reopen_interval = 5000
        self.reopen_timer = QTimer(self)
        self.reopen_timer.setSingleShot(True)

        # thread
        self.thread_item_model = QThread()
        self.thread_item_model.start()
        self.moveToThread(self.thread_item_model)

        # signal connection
        self.start_work.connect(self.detecting_items)
        self.reopen_timer.timeout.connect(self.trying2reopen_camera)
        ...
        self.parent = parent
        self.capture = camera_object  # cv2.VideoCapture class
        self.success2read_status = True
        self.fail2read_counter = 0
        self.recent_error_timestamp = 0.0
        self.mylogger1_3 = logging.getLogger(logger_name)
        ...
        if self.parent:
            self.mylogger1_3.info('ML model(item): ML model lives in %s' % self.thread())
            self.mylogger1_3.info('ML model(item): local thread is %s' % self.thread_item_model)
            self.mylogger1_3.info('ML model(item): QTimer lives in %s' % self.reopen_timer.thread())
            self.mylogger1_3.info('ML model(item): main thread is %s' % self.parent.thread())
        else:
            self.mylogger1_3.info('ML model(item): ML model lives in %s' % self.thread())
            self.mylogger1_3.info('ML model(item): local thread is %s' % self.thread_item_model)
            self.mylogger1_3.info('ML model(item): QTimer lives in %s' % self.reopen_timer.thread())
        self.mylogger1_3.info('The initialization of ML model successes')


    def trying2reopen_camera(self):
        self.mylogger1_3.info('%s: trying2reopen_camera starts' % self.__class__)
        # using the status which is related to the one using the cv2.VideoCapture
        if self.success2read_status:
            self.mylogger1_3.info('%s: self.success2read_status is True again, hence no need to reopen' % self.__class__)
        else:
            self.mylogger1_3.info('%s: try to open camera with path: %s' % (self.__class__, self.cam_path))
            # cv2.VideoCapture class
            self.capture.open(self.cam_path)  # It first calls VideoCapture.release to close the already opened camera.
            reopen_camera_result = self.capture.isOpened()
            if reopen_camera_result:
                self.mylogger1_3.info('%s: try to reopen camera and result is %s'
                                      % (self.__class__, reopen_camera_result))
                self.camera_error_info_update.emit(True)
            else:
                self.mylogger1_3.info('%s: try to reopen camera and result is %s'
                                      % (self.__class__, reopen_camera_result))
                self.camera_error_info_update.emit(False)
                self.reopen_timer.start(self.reopen_interval)
        self.mylogger1_3.info('%s: trying2reopen_camera ends' %self.__class__)

    def detecting_items(self):
        """
        :return: a list of detected products;
        """
        # print('ML Model begins')
        self.mylogger1_3.info('ML Model begins')
        time.sleep(0.2)  # to ensure the image is stable;
        discard_counter = 4
        while discard_counter > 0:
            self.capture.read()
        ...
        self.success2read_status, image = self.capture.read()
        if self.success2read_status:
            # success to retrieve a frame from the camera
            cv2.imwrite('./working_images_original/%s.jpg' % str(int(datetime.now().timestamp())), image)
            image2 = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            image_np_expanded = np.expand_dims(image2, axis=0)  # inserting a new axis at the position that you provide
            ...
            img, result = vis_util_hobin.visualize_boxes_and_labels_on_image_array(
                ...
                line_thickness=3)
            # print("ML Model (end): %s" % result)
            self.mylogger1_3.info("ML Model (end): %s" % result)
            detect_time = datetime.now()
            self.upload_img.emit(image, detect_time)
            self.detected.emit(result)
        else:
            # can not retrieve a frame from the camera
            current_error_timestamp = datetime.now().timestamp()
            if  current_error_timestamp - self.recent_error_timestamp < 60:
                self.fail2read_counter = self.fail2read_counter + 1
            else:
                self.recent_error_timestamp = current_error_timestamp
                self.fail2read_counter = 1

            self.mylogger1_3.info("""ML Model (end): fails to retrieve new frame for %s times and sets the detected
                                              result to be []""" % self.fail2read_counter)
            if self.fail2read_counter > 1:
                self.recent_error_timestamp = current_error_timestamp
                self.fail2read_counter = 0
                self.reopen_timer.start(10)
...
class Detection2_3_2_klas_singtel(Detection2_3_2):
    """The camera frame is refreshed by the my widget which is given in AllLayout_Singtel.py"""
    def __init__(self, parent, **kwargs):
        super(Detection2_3_2_klas_singtel, self).__init__(parent=parent, **kwargs)
    ...
    def detecting_ThumbsUp(self):
        ...
        the possible results are: 'GOOD', 'OTHER', 'OK'
        """
        ret, self.frame = self.parent.user_capture01.read()
        image = cv2.cvtColor(self.frame, cv2.COLOR_RGB2BGR)
        image_np_expanded = np.expand_dims(image, axis=0)
...
class Detection2_simulation_singtel(QThread):
    """
    The main difference of the simulation version of ML gesture model is the detected result is fixed;
    Compared with Detection2_simulation class, the usage of cv2.VideoCapture.read() is changed since the layout changes.
    """
    detected = pyqtSignal()
    detected_gesture_info = pyqtSignal(object, object)  # frame, gesture location information
    upload_image = pyqtSignal(object, object)
    ...
    def __init__(self, parent, logger_name='hobin', *,
                 ...
                 path_to_labels = 'D:\\01PythonFile\\tensorflow\\models-master\\research\\object_detection\\data\\hand_label_map.pbtxt',
                 num_classes = 3,  **kwargs):
        super(Detection2_simulation_singtel, self).__init__(parent)
        self.parent = parent
        self.frame = None
        self.status = True
        self.order_number = 'invalid'
            ...
            while self.status:
                ...
                if 'GOOD' in first_result:
                    ...
                    if self.parent.new_order_flag:
                        ...
                        continue
                    second_result = self.result2
                    second_result_location = [(0, 0, 100, 100), (0, 0, 100, 100), (0, 0, 100, 100), (0, 0, 100, 100), (0, 0, 100, 100)]
                    if 'GOOD' in second_result:
                        for index, detected_result in enumerate(second_result):
                            if detected_result == 'GOOD':
                                self.order_number = self.parent.thread6.dict02['data']['order_no']
...
class Detection1_multi_model_2(QObject):
    """
    It is used to detect products.
    Compared with the Detection1_multi_model_1 class, the super class becomes QObject instead of QThread.
    Compared with the Detection1_multi_model_1 class, it adds support to reopen the camera. self.recent_error_timestamp
    is used to avoid the execution of 'self.recent_error_timestamp=0' when reading camera successfully.
    """
    detected = pyqtSignal(object)
    upload_img = pyqtSignal(object, object)
    start_work = pyqtSignal()  # Its slot is self.detecting_items()
    ...
    def __init__(self, camera_object, camera_object_2, logger_name='hobin', *,
                 path_to_ckpt=r'D:\01PythonFile\Brindley\object_detection\output_inference_graph.pb\frozen_inference_graph.pb',
                 path_to_labels=r'D:\01PythonFile\Brindley\object_detection\output_inference_graph.pb\pascal_label_map.pbtxt',
                 num_classes=25, cam_path='0',
                 path_to_ckpt_2=r'D:\01PythonFile\Brindley\object_detection\output_inference_graph.pb\frozen_inference_graph.pb',
                 path_to_labels_2=r'D:\01PythonFile\Brindley\object_detection\output_inference_graph.pb\pascal_label_map.pbtxt',
                 num_classes_2=25, cam_path_2='1',
                 parent=None, **kwargs):
        super(Detection1_multi_model_2, self).__init__()
        # timer for reopen of camera
        self.reopen_interval = 5000
        self.reopen_timer = QTimer(self)
        self.reopen_timer.setSingleShot(True)
        ...
        self.list_model2 = ['425', '608', '3398', '3516', '11306', '2320', '3907', '141933', '140487', '141934']
        self.success2read_status = True
        self.success2read_status_2 = True
        self.fail2read_counter = 0
        self.recent_error_timestamp = 0.0
        self.mylogger_multi_model1_2 = logging.getLogger(logger_name)
        ##################################################################################################
        # main model
        PATH_TO_CKPT = path_to_ckpt
        PATH_TO_LABELS = path_to_labels
        ...
        ##################################################################################################
        if self.parent:
            self.mylogger_multi_model1_2.info('ML model(item): ML model lives in %s' % self.thread())
            self.mylogger_multi_model1_2.info('ML model(item): local thread is %s' % self.thread_item_model)
            self.mylogger_multi_model1_2.info('ML model(item): QTimer lives in %s' % self.reopen_timer.thread())
            self.mylogger_multi_model1_2.info('ML model(item): main thread is %s' % self.parent.thread())
        else:
            self.mylogger_multi_model1_2.info('ML model(item): ML model lives in %s' % self.thread())
            self.mylogger_multi_model1_2.info('ML model(item): local thread is %s' % self.thread_item_model)
            self.mylogger_multi_model1_2.info('ML model(item): QTimer lives in %s' % self.reopen_timer.thread())
        self.mylogger_multi_model1_2.info('The initialization of ML model successes')

    def trying2reopen_camera(self):
        self.mylogger_multi_model1_2.info('%s: trying2reopen_camera starts' % self.__class__)
        # using the status which is related to the one using the cv2.VideoCapture
        if self.success2read_status and self.success2read_status_2:
            self.mylogger_multi_model1_2.info(
                '%s: self.success2read_status and self.success2read_status_2 are True again, hence no need to reopen'
                % self.__class__)
        else:
            self.mylogger_multi_model1_2.info('%s: try to open camera with path: %s' % (self.__class__, self.cam_path))
            self.capture.open(self.cam_path)  # It first calls VideoCapture.release to close the already opened camera.
            reopen_camera_result = self.capture.isOpened()
            #
            self.mylogger_multi_model1_2.info('%s: try to open camera with path: %s' % (self.__class__, self.cam_path_2))
            self.capture_2.open(self.cam_path_2)
            reopen_camera_result_2 = self.capture_2.isOpened()
            if reopen_camera_result and reopen_camera_result_2:
                self.mylogger_multi_model1_2.info('%s: try to reopen camera and result is %s'
                                                  % (self.__class__, reopen_camera_result))
                self.camera_error_info_update.emit(True)
            else:
                self.mylogger_multi_model1_2.info('%s: try to reopen camera and result is %s'
                ...
                self.camera_error_info_update.emit(False)
                self.reopen_timer.start(self.reopen_interval)
        self.mylogger_multi_model1_2.info('%s: trying2reopen_camera ends' % self.__class__)
    ...
    def detecting_items(self):
        ...
        """
        # print('ML Model begins')
        self.mylogger_multi_model1_2.info('ML Model begins')
        time.sleep(0.2)  # to ensure the image is stable;
        discard_counter = 4
        while discard_counter > 0:
            self.capture.read()
            ...
            discard_counter = discard_counter - 1
        self.success2read_status, image = self.capture.read()
        self.success2read_status_2, image3 = self.capture_2.read()  # the fifth image probably is the newest one
        if self.success2read_status and self.success2read_status_2:
            # success to retrieve a frame from the camera
            cv2.imwrite('./working_images_original/%s_items1.jpg' % datetime.now().strftime('%Y%m%d_%Hh%Mm%Ss'), image)
            cv2.imwrite('./working_images_original/%s_items2.jpg' % datetime.now().strftime('%Y%m%d_%Hh%Mm%Ss'), image3)
            ...
            image_with_detected = copy.deepcopy(image)  # doing the copy because of the 'upload_img' signal
            img, result = vis_util_hobin.visualize_boxes_and_labels_on_image_array(
                image_with_detected,
                np.squeeze(boxes),
                np.squeeze(classes).astype(np.int32),
            ...
            # end of model detection
            self.mylogger_multi_model1_2.info(
                'ML Model (end): the first result is %s and the second result is %s.' % (result, result2))
            final = self.fusion(result, result2, ls_model1=self.list_model1, ls_model2=self.list_model2)
            self.mylogger_multi_model1_2.info("ML Model (end): %s" % final)
            detect_time = datetime.now()
            self.upload_img.emit(image, detect_time)
            self.detected.emit(final)
        else:
            ...
            self.mylogger_multi_model1_2.info("""ML Model (end): fails to retrieve new frame for %s times and sets the detected
                                              result to be []""" % self.fail2read_counter)
            if self.fail2read_counter > 1:
                self.recent_error_timestamp = current_error_timestamp
                self.fail2read_counter = 0
...
class Detection1_multi_model_2_1(QObject):
    """
    It is used to detect products.
    Compared with the Detection1_multi_model_2 class, this class changes the initialization and the utilization of tensorflow model.
    """
    detected = pyqtSignal(object)
    upload_img = pyqtSignal(object, object)
    start_work = pyqtSignal()  # Its slot is self.detecting_items()
    ...
    def __init__(self, camera_object, camera_object_2, logger_name='hobin', *,
                 box_pb_path=r'./box.pb', cls_pb_path=r'./cls.pb', sku_classes=r'./classes.txt', cam_path='0',
                 box_pb_path_2=r'./box.pb', cls_pb_path_2=r'./cls.pb', sku_classes_2=r'./classes.txt', cam_path_2='1',
                 parent=None, **kwargs):
        super(Detection1_multi_model_2_1, self).__init__()
        # timer for reopen of camera
        self.reopen_interval = 5000
        self.reopen_timer = QTimer(self)
        self.reopen_timer.setSingleShot(True)
        ...
        # model path
        self.box_pb_path = box_pb_path  # written from configure file
        self.cls_pb_path = cls_pb_path  # written from configure file
        self.sku_classes = np.loadtxt(sku_classes, dtype=str).tolist()  # written from configure file

        self.box_pb_path_2 = box_pb_path_2  # written from configure file
        self.cls_pb_path_2 = cls_pb_path_2  # written from configure file
        self.sku_classes_2 = np.loadtxt(sku_classes_2, dtype=str).tolist()  # written from configure file
        ...
        self.fail2read_counter = 0
        self.recent_error_timestamp = 0.0
        self.mylogger_multi_model1_2_1 = logging.getLogger(logger_name)
        ##################################################################################################
        # main model
        self.g1 = tf.Graph()
        self.g2 = tf.Graph()
        ...
        self.sess1 = tf.Session(config=config, graph=self.g1)
        self.sess2 = tf.Session(config=config, graph=self.g2)

        (self.step1_inputs, self.step1_boxes, self.step1_classes,
         self.step1_scores) = self.load_step1(self.g1, self.box_pb_path, self.sess1)
        self.step2_inputs, self.step2_scores = self.load_step2(self.g2, self.cls_pb_path, self.sess2)
        ...
        config = tf.ConfigProto(allow_soft_placement=True)
        config.gpu_options.per_process_gpu_memory_fraction = 0.3
        self.sess3 = tf.Session(config=config, graph=self.g3)
        self.sess4 = tf.Session(config=config, graph=self.g4)

        (self.step1_inputs_2, self.step1_boxes_2, self.step1_classes_2,
         self.step1_scores_2) = self.load_step1(self.g3, self.box_pb_path_2, self.sess3)
        self.step2_inputs_2, self.step2_scores_2 = self.load_step2(self.g4, self.cls_pb_path_2, self.sess4)
        # self.mylogger = logging.getLogger(logger_name)
        print('The initialization of ML model successes')
        ##################################################################################################
        if self.parent:
            self.mylogger_multi_model1_2_1.info('ML model(item): ML model lives in %s' % self.thread())
            self.mylogger_multi_model1_2_1.info('ML model(item): local thread is %s' % self.thread_item_model)
            self.mylogger_multi_model1_2_1.info('ML model(item): QTimer lives in %s' % self.reopen_timer.thread())
            self.mylogger_multi_model1_2_1.info('ML model(item): main thread is %s' % self.parent.thread())
        else:
            self.mylogger_multi_model1_2_1.info('ML model(item): ML model lives in %s' % self.thread())
            self.mylogger_multi_model1_2_1.info('ML model(item): local thread is %s' % self.thread_item_model)
            self.mylogger_multi_model1_2_1.info('ML model(item): QTimer lives in %s' % self.reopen_timer.thread())
        self.mylogger_multi_model1_2_1.info('The initialization of ML model successes')

    def trying2reopen_camera(self):
        self.mylogger_multi_model1_2_1.info('%s: trying2reopen_camera starts' % self.__class__)
        # using the status which is related to the one using the cv2.VideoCapture
        if self.success2read_status and self.success2read_status_2:
            self.mylogger_multi_model1_2_1.info(
                '%s: self.success2read_status and self.success2read_status_2 are True again, hence no need to reopen'
                % self.__class__)
        else:
            self.mylogger_multi_model1_2_1.info('%s: try to open camera with path: %s' % (self.__class__, self.cam_path))
            self.capture.open(self.cam_path)  # It first calls VideoCapture.release to close the already opened camera.
            reopen_camera_result = self.capture.isOpened()
            #
            self.mylogger_multi_model1_2_1.info(
                '%s: try to open camera with path: %s' % (self.__class__, self.cam_path_2))
            self.capture_2.open(self.cam_path_2)
            reopen_camera_result_2 = self.capture_2.isOpened()
            if reopen_camera_result and reopen_camera_result_2:
                self.mylogger_multi_model1_2_1.info('%s: try to reopen camera and result is %s'
                                                  % (self.__class__, reopen_camera_result))
                self.camera_error_info_update.emit(True)
            else:
                self.mylogger_multi_model1_2_1.info('%s: try to reopen camera and result is %s'
                ...
                self.camera_error_info_update.emit(False)
                self.reopen_timer.start(self.reopen_interval)
        self.mylogger_multi_model1_2_1.info('%s: trying2reopen_camera ends' % self.__class__)

    def fusion(self, result1, result2, ls_model1=(), ls_model2=()):
        """
        This function is provided by siwei;
        :param result1: result from 1st camera
        :param result2: result from 1st camera
        :param ls_model1:
        :param ls_model2: select result from 'result2' where result in 'ls_model2'
        :return: final result
        """
        final = []
        for rlt in result1:
    ...
    def detecting_items(self):
        ...
        """
        # print('ML Model begins')
        self.mylogger_multi_model1_2_1.info('ML Model begins')
        time.sleep(0.2)  # to ensure the image is stable;
        discard_counter = 4
        while discard_counter > 0:
            self.capture.read()
        ...
        if self.success2read_status and self.success2read_status_2:
            ...
            # detection of model 1
            detect_dict = {}
            detect_dict['restrict_line'] = np.array([[387, 182, 162, 488],
                                                     [162, 488, 427, 929],
                                                     [427, 929, 767, 677],
                                                     [767, 562, 387, 182]], dtype=np.float32)
            detect_dict['image_bgr'] = image
            detect_dict['image_rgb'] = cv2.cvtColor(detect_dict['image_bgr'], cv2.COLOR_BGR2RGB)
            feed_dict1 = {self.step1_inputs: np.expand_dims(detect_dict['image_rgb'], axis=0)}
            (detect_dict['step1_boxes'], detect_dict['step1_classes'],
             detect_dict['step1_scores']) = self.sess1.run([self.step1_boxes, self.step1_classes, self.step1_scores],
                                                           feed_dict=feed_dict1)

            detect_dict = self.step1_result_process(detect_dict, thr=0.8)
            ...
            for i in range(num_step1_detect_boxes):
                ...
                pred = self.sess2.run([self.step2_scores], feed_dict={self.step2_inputs: self.resize_image(_image)})
                ...
                step2_skus_list.append(self.sku_classes[class_index])
            ...
            detect_dict['step2_skus'] = np.array(step2_skus_list, dtype=np.str)
            detect_dict = self.step2_result_nms(detect_dict, max_ovr=0.5)
            detect_dict = self.region_restriction(detect_dict)
            detect_dict = self.draw_image(detect_dict)

            result = detect_dict['step2_skus'][
                np.where(detect_dict['step2_nms_keeps'] + detect_dict['step1_region_restrict'] == 2)]
            result = result[np.where(result != '000000')]
            cv2.imwrite('./working_images/%s_items1.jpg' % datetime.now().strftime('%Y%m%d_%Hh%Mm%Ss'),
                        detect_dict['image_bgr'])
            ...
            detect_dict['restrict_line'] = np.array([[387, 182, 162, 488],
                                                     ...
                                                     [427, 929, 767, 677],
                                                     [767, 562, 387, 182]], dtype=np.float32)
            detect_dict['image_bgr'] = image3
            detect_dict['image_rgb'] = cv2.cvtColor(detect_dict['image_bgr'], cv2.COLOR_BGR2RGB)
            feed_dict1 = {self.step1_inputs_2: np.expand_dims(detect_dict['image_rgb'], axis=0)}
            (detect_dict['step1_boxes'], detect_dict['step1_classes'],
             detect_dict['step1_scores']) = self.sess3.run(
                [self.step1_boxes_2, self.step1_classes_2, self.step1_scores_2],
                feed_dict=feed_dict1)
            ...
            for i in range(num_step1_detect_boxes):
                box_i = detect_dict['step1_boxes'][i]
                _image = detect_dict['image_rgb'][int(box_i[0]):int(box_i[2]), int(box_i[1]):int(box_i[3]), :]
                pred = self.sess4.run([self.step2_scores_2],
                                      feed_dict={self.step2_inputs_2: self.resize_image(_image)})
                class_index = np.argmax(pred[0], 1)[0]
                max_scores = pred[0][0][class_index]
                step2_classes_list.append(class_index)
                step2_scores_list.append(max_scores)
                step2_skus_list.append(self.sku_classes_2[class_index])
            detect_dict['step2_scores'] = np.array(step2_scores_list, dtype=np.float32)
            detect_dict['step2_classes'] = np.array(step2_classes_list, dtype=np.int32)
            detect_dict['step2_skus'] = np.array(step2_skus_list, dtype=np.str)
            detect_dict = self.step2_result_nms(detect_dict, max_ovr=0.5)
            ...
            result2 = detect_dict['step2_skus'][
                np.where(detect_dict['step2_nms_keeps'] + detect_dict['step1_region_restrict'] == 2)]
            result2 = result2[np.where(result != '000000')]
            cv2.imwrite('./working_images/%s_items2.jpg' % datetime.now().strftime('%Y%m%d_%Hh%Mm%Ss'),
                        detect_dict['image_bgr'])
            ...
            final = self.fusion(result, result2, ls_model1=self.list_model1, ls_model2=self.list_model2)
            self.mylogger_multi_model1_2_1.info("ML Model (end): %s" % final)
            detect_time = datetime.now()
            self.upload_img.emit(image, detect_time)
            self.detected.emit(final)
        else:
            ...
            self.mylogger_multi_model1_2_1.info("""ML Model (end): fails to retrieve new frame for %s times and sets the detected
                                              result to be []""" % self.fail2read_counter)
            if self.fail2read_counter > 1:
                self.recent_error_timestamp = current_error_timestamp
                self.fail2read_counter = 0
    ...
    def load_step1(self, g, pb_path, sess):
    ...
    def load_step2(self, g, pb_path, sess):
        with sess.as_default():
            with g.as_default():
                gx = tf.GraphDef()
                with tf.gfile.GFile(pb_path, 'rb') as fid:
    ...
    def step1_result_process(self, detect_dict, thr=0.8):
    ...
    def nms(self, box, scores, max_ovr=0.5):
        x1 = box[:, 1]
        y1 = box[:, 0]
        x2 = box[:, 3]
        y2 = box[:, 2]
        ...
        while order.size > 0:
            ...
            # ovr = inter/min(areas[i] + areas[order[1:]] - inter)
            inds = np.where(ovr <= max_ovr)[0]
            order = order[inds + 1]
        return keep

    def step2_result_nms(self, detect_dict, max_ovr=0.5):
        ...
        if num < len(detect_dict['step1_classes']):
            for i in range(num):
                ...
                keep = self.nms(box_i, score_i, max_ovr)
    ...
    def draw_image(self, detect_dict):
        ...
        for i in range(len(detect_dict['step1_classes'])):
            if detect_dict['step2_nms_keeps'][i] == 1:
                ...
                cv2.putText(image, text1, (int(box[1]), int(box[0]) + 10), 0, 0.5, (0, 0, 0), 2)
                cv2.putText(image, text2, (int(box[1]), int(box[0]) + 30), 0, 0.5, (255, 0, 0), 2)
        for restrict_line in detect_dict['restrict_line']:
            cv2.line(image, (restrict_line[1], restrict_line[0]), (restrict_line[3], restrict_line[2]), (0, 255, 0), 3)
        return detect_dict

    def region_restriction(self, detect_dict, region_xy_core=[384, 512]):
        num_boxes = len(detect_dict['step1_classes'])
        keeps = np.ones(num_boxes, dtype=np.int8)
        box = detect_dict['step1_boxes']
        box_xy = [[box[i][2], int((box[i][1] + box[i][3]) / 2)] for i in range(num_boxes)]
        restrict_line = detect_dict['restrict_line']
        for j in range(num_boxes):
            x = box_xy[j][1]
            y = box_xy[j][0]
            cond = []
            for i in range(len(restrict_line)):
                x1 = restrict_line[i][1]
                y1 = restrict_line[i][0]
                x2 = restrict_line[i][3]
                y2 = restrict_line[i][2]
                f = (x - x2) / (x1 - x2) - (y - y2) / (y1 - y2)
                if (region_xy_core[1] - x2) / (x1 - x2) - (region_xy_core[0] - y2) / (y1 - y2) < 0:
                    cond.append(f <= 0)
                else:
                    cond.append(f > 0)
            if not all(cond):
                keeps[j] = 0
        detect_dict['step1_region_restrict'] = keeps
        return detect_dict
