...
class MyThread9_1_1(QThread):
    """
    It is used to upload images after the payment is finished.
    The server only accepts one image at one time.
    Compared with the MyThread9 class, the logging module is introduced to this module at first time.
    Compared with the MyThread9 class, the original header of requests is reloaded by the self.headers01 (the 'Connection' header changes from keep-alive to close);
    Compared with the MyThread9_1 class, the file format of the upload file is changed to append the suffix of file;
    """
    error = pyqtSignal()

    def __init__(self, parent=None, logger_name='hobin', *, url = 'http://api.commaai.cn/order/attach/upload_payment_attach',
                 sign_key='4b111cc14a33b88e37e2e2934f493458', utm_medium = 'app', utm_source = 'box', **kwargs):
        super(MyThread9_1_1, self).__init__(parent)
        # the first way to configuring the parameters, almost all parameters get their value from those named keyword arguments.
        self.url = url
        self.sign_key = sign_key
        # if attach_type =1, it indicates the image is related to the user.
        ...
        # some variables
        self.parent = parent
        self.mylogger9_1_1 = logging.getLogger(logger_name)
        self.img_user = None
        self.img_gesture = None
        self.img_items = None
        self.headers01 = {'User-Agent': 'python-requests', 'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate',
        ...
        self.files = {'picture': None}
        self.dict02 = {}  # the dictionary object of the first HTTP result.
        self.mylogger9_1_1.info('The initialization of image-upload thread is successful.')
    ...
    def run(self):
        # print('Image-upload thread begins')
        self.mylogger9_1_1.info('Image-upload thread begins')
        # the value of self.dict01['order_no'] is given before the run function and it is 'str' type;
        # the value of self.image is given before the run function and it is 'str' type;
        try:
            # prepare to upload the first image
            ...
            self.dict01['attach_type'] = '1'
            ret, buffer = cv2.imencode('.jpg', self.img_user)
            self.files['picture'] = ('user.jpg',buffer.tostring(),'image/jpeg')
            self.dict01['api_sign'] = self.api_sign_hexdigest(self.dict01)
            resp01 = requests.post(self.url, headers=self.headers01, files=self.files, data=self.dict01, timeout=8)
            if resp01.status_code == 200:
                self.dict02 = json.loads(resp01.text)
                # print('Image-upload thread with type %s gets: %s' % (self.dict01['attach_type'],self.dict02))
                self.mylogger9_1_1.info('Image-upload thread with type %s gets %s' % (self.dict01['attach_type'],self.dict02))
            else:
                # print('Image-upload thread with type %s (end): network problem.' % self.dict01['attach_type'])  # E.g. 404
                self.mylogger9_1_1.info('Image-upload thread with type %s (end): network problem.' % self.dict01['attach_type'])
            # prepare to upload the second image
            self.dict01['attach_type'] = '2'
            ret, buffer = cv2.imencode('.jpg', self.img_gesture)
            self.files['picture'] = ('gesture.jpg',buffer.tostring(),'image/jpeg')
            self.dict01['api_sign'] = self.api_sign_hexdigest(self.dict01)
            resp02 = requests.post(self.url, headers=self.headers01, files=self.files, data=self.dict01, timeout=8)
            if resp02.status_code == 200:
                self.dict02 = json.loads(resp02.text)
            ...
            self.dict01['attach_type'] = '3'
            ret, buffer = cv2.imencode('.jpg', self.img_items)
            self.files['picture'] = ('items.jpg',buffer.tostring(),'image/jpeg')
            self.dict01['api_sign'] = self.api_sign_hexdigest(self.dict01)
            resp03 = requests.post(self.url, headers=self.headers01, files=self.files, data=self.dict01, timeout=8)
            if resp03.status_code == 200:
                self.dict02 = json.loads(resp03.text)
                ...
                self.mylogger9_1_1.info('Image-upload thread with type %s (end): network problem.' % self.dict01['attach_type'])
            # print('Image-upload thread ends')
            self.mylogger9_1_1.info('Image-upload thread ends')
        except BaseException as e:
            # self.mylogger9_1_1.error(e)
            self.mylogger9_1_1.error('-------------------------Error happens in Image-upload thread.----------------------------', exc_info=True)
            self.error.emit()


class MyThread9_1_2(QThread):
    """
    Compared with MyThread9_1_1 class, this class adds extra information to self.dict01:
    client_ip, csp_id, system_version, api_ver, device_id, store_id
    """
    error = pyqtSignal()

    def __init__(self, parent=None, client_ip='127.0.0.1', logger_name='hobin', *, csp_id='cashier_0001',
                 system_version='windows', api_ver='1.0', device_id='cashier', store_id='2',
                 url='http://api.commaai.cn/order/attach/upload_payment_attach',
                 sign_key='4b111cc14a33b88e37e2e2934f493458', utm_medium='app', utm_source='box', **kwargs):
        super(MyThread9_1_2, self).__init__(parent)
        # the first way to configuring the parameters, almost all parameters get their value from those named keyword arguments.
        self.url = url
        self.sign_key = sign_key
        # if attach_type =1, it indicates the image is related to the user.
        ...
        # some variables
        self.parent = parent
        self.mylogger9_1_2 = logging.getLogger(logger_name)
        self.img_user = None
        self.img_gesture = None
        self.img_items = None
        self.headers01 = {'User-Agent': 'python-requests', 'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate',
        ...
        self.files = {'picture': None}
        self.dict02 = {}  # the dictionary object of the first HTTP result.
        self.mylogger9_1_2.info('The initialization of image-upload thread is successful.')
    ...
    def run(self):
        # print('Image-upload thread begins')
        self.mylogger9_1_2.info('Image-upload thread begins')
        # the value of self.dict01['order_no'] is given before the run function and it is 'str' type;
        # the value of self.image is given before the run function and it is 'str' type;
        try:
            # prepare to upload the first image
            ...
            if resp01.status_code == 200:
                self.dict02 = json.loads(resp01.text)
                # print('Image-upload thread with type %s gets: %s' % (self.dict01['attach_type'],self.dict02))
                self.mylogger9_1_2.info(
                    'Image-upload thread with type %s gets %s' % (self.dict01['attach_type'], self.dict02))
            else:
                # print('Image-upload thread with type %s (end): network problem.' % self.dict01['attach_type'])  # E.g. 404
                self.mylogger9_1_2.info(
                    'Image-upload thread with type %s (end): network problem.' % self.dict01['attach_type'])
            # prepare to upload the second image
            self.dict01['attach_type'] = '2'
            ret, buffer = cv2.imencode('.jpg', self.img_gesture)
            self.files['picture'] = ('gesture.jpg', buffer.tostring(), 'image/jpeg')
            ...
            if resp03.status_code == 200:
                ...
                self.mylogger9_1_2.info(
                    'Image-upload thread with type %s (end): network problem.' % self.dict01['attach_type'])
            # print('Image-upload thread ends')
            self.mylogger9_1_2.info('Image-upload thread ends')
        except BaseException as e:
            # self.mylogger9_1_2.error(e)
            self.mylogger9_1_2.error(
                '-------------------------Error happens in Image-upload thread.----------------------------',
                exc_info=True)
            self.error.emit()
...
if __name__ == '__main__':
    ...
    b = MyThread9_1_2(**myconfig.data['image_upload'])
    b.dict01['order_no'] = 'SO201807311833400134'
    b.img_user = cv2.imread(r'./upload/test.jpg')
    b.img_gesture = cv2.imread(r'./upload/test.jpg')
    b.img_items = cv2.imread(r'./upload/test.jpg')
    b.start()
    while b.isRunning():
        pass
