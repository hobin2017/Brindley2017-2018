...
"""
Payment System
Compared with Klas_Singtel_V1_1_1, MyThread7_1_1 is replaced by MyThread7_1_1_klas_singtel;
author: hobin;
...
from GesturePayThread import MyThread7_1_1_klas_singtel
...
class PaymentSystem_klas(QObject):
    ...
    def __init__(self, **kwargs):
        ...
        self.thread5.user_info_success.connect(self.account_work6_klas_singtel)
        ...
        self.thread6.order_error.connect(self.order_error07)

        # thread8 is about the gesture-pay thread
        self.error_try_times_gesture_pay = 0
        self.thread8 = MyThread7_1_1_klas_singtel(client_ip=self.current_ip_address, **self.myconfig.data['gesture_pay'])
        ...
        self.thread8.order_success.connect(self.websocket_work11_klas)
    ...
    def account_work6_klas_singtel(self, user_name, user_img, wechat_entrust, user_face):
        """
        ...
        :return:
        """
        self.mylogging.logger.info('account_work6_klas_singtel begins.')
        ...
        if wechat_entrust == 1:
            self.mylogging.logger.info('account_work6_klas_singtel detects that the user can show gesture to finish the payment.')
            ...
            if self.user_id == 'invalid' or self.order_number == 'invalid':
                self.mylogging.logger.info('account_work6_klas_singtel (end): the user id or the order number is invalid.')
                ...
                if self.order_number == self.thread0_0.order_number:
                    self.thread8.success_order_number4image_upload = self.thread0_0.order_number
                    self.thread8.dict01['user_id'] = self.user_id
                    self.thread8.dict01['order_no'] = self.order_number
                    self.mylogging.logger.info('account_work6_klas_singtel (end): order is ready to be submitted.')
                    self.thread8.start()
                else:
                    self.mylogging.logger.info('account_work6_klas_singtel (end): the current order number is different from the order number recorded by the gesture ML Model.')
                    self.main_window.toSettlement()
                    if self.gesture_frame_frozen_status:
                        # although the account thread detects only one user but now something goes wrong.
                        self.main_window.stopHandGif()
            ...
            self.mylogging.logger.info('account_work6_klas_singtel (end) : the user can not show gesture.')
