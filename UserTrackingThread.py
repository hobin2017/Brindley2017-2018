...
class MyThread6(QThread):
    ...
    def run(self):
        ...
        print('User checking thread begins')
        ...
        if len(faces) > 0:
            for index, (x, y, w, h) in enumerate(faces):
                ...
                if self.faceNotFound:
                    ...
                    print('User checking thread with Face Detection (end): the first person detected by the algorithm becomes the primary user.')
                    break
                    ...
                    print('User checking thread with Face Detection (end): the primary user with index %s is detected successfully.' % index)
                    self.priorCenter = currentCenter
                    self.faceNotFound = False
                    break
                if index == len(faces) - 1:
                    self.faceNotFound = True
                    print('User checking thread with Face Detection (end): the primary user is gone.')
        else:
            ...
            if len(eyes) > 0:
                ...
                if self.faceNotFound:
                    ...
                    self.faceNotFound = False
                    print('User checking thread with Eyes Detection (end): the first person detected by the algorithm becomes the primary user.')
                    ...
                    self.priorCenter = center_face_possible
                    self.faceNotFound = False
                    print('User checking thread with Eyes Detection (end): detecting the primary user successfully.')
                else:
                    self.faceNotFound = True
                    print('User checking thread with Eyes Detection (end): the primary user is gone.')

            else:
                self.faceNotFound = True
                print('User checking thread with Eyes & Face Detection (end): no user is detected.')
                ...
                if len(faces) > 0:
                    for index, (x, y, w, h) in enumerate(faces):
                        ...
                        if self.faceNotFound:
                            ...
                            # Hopefully the index is 0 since the primary user is user with the biggest face;
                            print(
                                'User checking thread with Face Detection (end): the primary user with index %s is detected successfully.' % index)
                            self.priorCenter = currentCenter
                            self.faceNotFound = False
                            break
                        if index == len(faces) - 1:
                    ...
                    if len(eyes) > 0:
                        ...
                        if self.faceNotFound:
                            ...
                            self.faceNotFound = False
                            print(
                                'User checking thread with Eyes Detection (end): the first person detected by the algorithm becomes the primary user.')
                        elif abs(center_face_possible[0] - self.priorCenter[0]) < self.frame_width // 6 and abs(
                                center_face_possible[1] - self.priorCenter[1]) < self.frame_height // 6:
                            if abs(center_face_possible[0] - self.priorCenter[0]) < 7 and abs(
                                    center_face_possible[1] - self.priorCenter[1]) < 7:
                            ...
                            self.faceNotFound = False
                            print(
                                'User checking thread with Eyes Detection (end): detecting the primary user successfully.')
                        else:
                            self.faceNotFound = True
                            print('User checking thread with Eyes Detection (end): the primary user is gone.')
