...
class MyThread7_1_1(QThread):
    """
    Gesture order thread
    Compared with the MyThread7_1 class, this class adds extra parameters into self.dict01:
    csp_id, utm_medium, utm_source, system_version, api_ver, client_time(called time), device_id, client_ip
    Compared with the MyThread7_1 class, the hash algorithm changes from md5 to sha256.
    """
    order_success = pyqtSignal() # the initial purpose is to eliminate the magnetic of item;
    finished = pyqtSignal(object)
    finished_error = pyqtSignal()
    ...
    def __init__(self, parent=None, client_ip='127.0.0.1', logger_name='hobin', *, url = 'https://sys.commaai.cn/payment/pappay/pay',
                 sign_key='4b111cc14a33b88e37e2e2934f493458', store_id = '2', screen_id = '1',
                 csp_id='cashier_001', utm_medium='app', utm_source='box', system_version='windows', api_ver='1.0',
                 device_id='cashier', **kwargs):
        super(MyThread7_1_1, self).__init__(parent)
        # the first way to configuring the parameters, almost all parameters get their value from those named keyword arguments.
        self.url = url
        self.sign_key = sign_key
        ...
        # some variables
        self.parent = parent
        self.mylogger7_1_1 = logging.getLogger(logger_name)
        self.headers01 = {'User-Agent': 'python-requests', 'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate',
                          'Connection': 'close'}
        self.resp01 = None
        self.dict02 = {}  # the dictionary object of the HTTP result
        self.mylogger7_1_1.info('The initialization of gesture order thread is successful.')
    ...
    def run(self):
        try:
            # print('Gesture order thread begins')
            self.mylogger7_1_1.info('Gesture order thread begins')
            self.dict01['client_time'] = str(int(datetime.now().timestamp()))
            self.dict01['api_sign'] = self.api_sign_hexdigest(dict=self.dict01)
            self.mylogger7_1_1.info('Gesture order thread: the header is %s and the data part is %s' % (self.headers01, self.dict01))
            self.resp01 = requests.post(self.url, headers=self.headers01, data=self.dict01, timeout=8)
            if self.resp01.status_code == 200:
                self.dict02 = json.loads(self.resp01.text)
                self.mylogger7_1_1.info('Gesture order thread: %s' %self.dict02)
                if self.dict02['code'] == 200:
                    # print('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.mylogger7_1_1.info('Gesture order thread (end): %s.' % self.dict02['msg'])
                    # self.order_success.emit()
                elif self.dict02['code'] == 100:
                    # print('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.mylogger7_1_1.info('Gesture order thread (end): %s.' % self.dict02['msg'])
                ...
                # print('Gesture order thread: network problem occurs.')
                self.mylogger7_1_1.info('Gesture order thread: network problem occurs.')
                self.network_error.emit()
        except requests.exceptions.ReadTimeout:
            # print('***--------------Network problem happens in gesture order thread (Timeout for the HTTP request).------------------------***')
            self.mylogger7_1_1.error('***--------------Network problem happens in gesture order thread (Timeout for the HTTP request).------------------------***')
            self.timeout_http.emit()
        except requests.exceptions.ConnectionError:
            self.mylogger7_1_1.error('***--------------Network problem happens in gesture order thread.------------------------***',exc_info=True)
            self.connection_error.emit()
        except BaseException as e:
            # self.mylogger7_1_1.error(e)
            self.mylogger7_1_1.error('***------------------------Be careful! Error occurs in gesture order thread!------------------------***',exc_info=True)
            self.error.emit()


class MyThread7_1_1_klas_singtel(QThread):
    """
    Gesture order thread
    Compared with the MyThread7_1_1 class, the 'order_success' signal is emitted again to simulate receiving the
    successful gesture payment for websocket;
    """
    order_success = pyqtSignal(object) # order number
    finished = pyqtSignal(object)
    finished_error = pyqtSignal()
    network_error = pyqtSignal()
    timeout_http = pyqtSignal()
    ...
    def __init__(self, parent=None, client_ip='127.0.0.1', logger_name='hobin', *, url = 'https://sys.commaai.cn/payment/pappay/pay',
                 ...
                 csp_id='cashier_001', utm_medium='app', utm_source='box', system_version='windows', api_ver='1.0',
                 device_id='cashier', **kwargs):
        super(MyThread7_1_1_klas_singtel, self).__init__(parent)
        # the first way to configuring the parameters, almost all parameters get their value from those named keyword arguments.
        self.url = url
        self.sign_key = sign_key
        self.dict01 = {"api_sign": None,
        ...
        # some variables
        self.success_order_number4image_upload = None # like 'SO201901090000000000', taken by 'order_success' signal
        self.parent = parent
        self.mylogger7_1_1_klas_singtel = logging.getLogger(logger_name)
        self.headers01 = {'User-Agent': 'python-requests', 'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate',
                          'Connection': 'close'}
        self.resp01 = None
        self.dict02 = {}  # the dictionary object of the HTTP result
        self.mylogger7_1_1_klas_singtel.info('The initialization of gesture order thread is successful.')
    ...
    def run(self):
        try:
            # print('Gesture order thread begins')
            self.mylogger7_1_1_klas_singtel.info('Gesture order thread begins')
            self.dict01['client_time'] = str(int(datetime.now().timestamp()))
            self.dict01['api_sign'] = self.api_sign_hexdigest(dict=self.dict01)
            self.mylogger7_1_1_klas_singtel.info('Gesture order thread: the header is %s and the data part is %s' % (self.headers01, self.dict01))
            self.resp01 = requests.post(self.url, headers=self.headers01, data=self.dict01, timeout=8)
            if self.resp01.status_code == 200:
                self.dict02 = json.loads(self.resp01.text)
                self.mylogger7_1_1_klas_singtel.info('Gesture order thread: %s' %self.dict02)
                if self.dict02['code'] == 200:
                    # print('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.mylogger7_1_1_klas_singtel.info('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.order_success.emit(self.success_order_number4image_upload)
                elif self.dict02['code'] == 100:
                    # print('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.mylogger7_1_1_klas_singtel.info('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.finished_error.emit()
                ...
                # print('Gesture order thread: network problem occurs.')
                self.mylogger7_1_1_klas_singtel.info('Gesture order thread: network problem occurs.')
                self.network_error.emit()
        except requests.exceptions.ReadTimeout:
            # print('***--------------Network problem happens in gesture order thread (Timeout for the HTTP request).------------------------***')
            self.mylogger7_1_1_klas_singtel.error('***--------------Network problem happens in gesture order thread (Timeout for the HTTP request).------------------------***')
            self.timeout_http.emit()
        except requests.exceptions.ConnectionError:
            self.mylogger7_1_1_klas_singtel.error('***--------------Network problem happens in gesture order thread.------------------------***',exc_info=True)
            self.connection_error.emit()
        except BaseException as e:
            # self.mylogger7_1_1_klas_singtel.error(e)
            self.mylogger7_1_1_klas_singtel.error('***------------------------Be careful! Error occurs in gesture order thread!------------------------***',exc_info=True)
            self.error.emit()


class MyThread7_1_2(QThread):
    """
    Gesture order thread
    Compared with MyThread7_1, this class use the order_success01 signal to start Ask4PaymentResult thread;
    """
    order_success01 = pyqtSignal()
    finished = pyqtSignal(object)
    finished_error = pyqtSignal()
    network_error = pyqtSignal()
    timeout_http = pyqtSignal()
    ...
    def __init__(self, parent=None, logger_name='hobin', *, url = 'https://sys.commaai.cn/payment/pappay/pay',
                 sign_key='4b111cc14a33b88e37e2e2934f493458',
                 store_id = '2', screen_id = '1', **kwargs):
        super(MyThread7_1_2, self).__init__(parent)
        # the first way to configuring the parameters, almost all parameters get their value from those named keyword arguments.
        self.url = url
        self.sign_key = sign_key
        self.dict01 = {"token": None,
        ...
        # some variables
        self.parent = parent
        self.mylogger7_1_2 = logging.getLogger(logger_name)
        self.headers01 = {'User-Agent': 'python-requests', 'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate',
                          'Connection': 'close'}
        self.resp01 = None
        self.dict02 = {}  # the dictionary object of the HTTP result
        self.mylogger7_1_2.info('The initialization of gesture order thread is successful.')
    ...
    def run(self):
        try:
            # print('Gesture order thread begins')
            self.mylogger7_1_2.info('Gesture order thread begins')
            self.dict01['time'] = str(int(datetime.now().timestamp()))
            self.dict01['token'] = self.api_sign02_hexdigest()
            self.mylogger7_1_2.info('Gesture order thread: the header is %s and the data part is %s' % (self.headers01, self.dict01))
            self.resp01 = requests.post(self.url, headers=self.headers01, data=self.dict01, timeout=8)
            if self.resp01.status_code == 200:
                self.dict02 = json.loads(self.resp01.text)
                if self.dict02['code'] == 200:
                    # print('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.mylogger7_1_2.info('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.order_success01.emit()
                elif self.dict02['code'] == 100:
                    # print('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.mylogger7_1_2.info('Gesture order thread (end): %s.' % self.dict02['msg'])
                    self.finished_error.emit()
                ...
                # print('Gesture order thread: network problem occurs.')
                self.mylogger7_1_2.info('Gesture order thread: network problem occurs.')
                self.network_error.emit()
        except requests.exceptions.ReadTimeout:
            # print('***--------------Network problem happens in gesture order thread (Timeout for the HTTP request).------------------------***')
            self.mylogger7_1_2.error('***--------------Network problem happens in gesture order thread (Timeout for the HTTP request).------------------------***')
            self.timeout_http.emit()
        except requests.exceptions.ConnectionError:
            self.mylogger7_1_2.error('***--------------Network problem happens in gesture order thread.------------------------***',exc_info=True)
            self.connection_error.emit()
        except BaseException as e:
            # self.mylogger7_1_2.error(e)
            self.mylogger7_1_2.error('***------------------------Be careful! Error occurs in gesture order thread!------------------------***',exc_info=True)
            self.error.emit()
